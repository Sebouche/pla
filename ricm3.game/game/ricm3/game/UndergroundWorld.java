	...
	void generate_level() {
		...
		for (int i = 0; i < 15; i++) {
			...
			Block b = new Block(m_model, (int) (rand_x * Options.Entity_size * Options.Scale),
					...
					new IAutomaton(Options.selectedAutomata.get(0)), m_model.m_undergroundworld);
			b.set_idsprite(8);
			m_entities.add(b);
		}
		// Generation maison
				...
				if (m_grid[i][j] == null) {
						...
						for (int k = -1; k <= 1; k++) {
							...
							if (randint2 <= 3 && i + k >= 0 && i + k < 20) {
								b = new Copper(m_model, (int) (j * Options.Entity_size * Options.Scale),
										...
										m_model.m_sprites.get("block"), new IAutomaton(Options.selectedAutomata.get(0)),
					...
					if (randint >= 999 && i >= 30) {
						...
						m_grid[i][j] = new Water(m_model, (int) (j * (Options.Entity_size * Options.Scale)),
								(int) (i * (Options.Entity_size * Options.Scale)),  m_rand.nextInt(12), m_model.m_sprites.get("block"),
								Options.selectedAutomata.get(10), this);
						;
